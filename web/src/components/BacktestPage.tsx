...
const findClosestIndexSorted = (
  ...
  return target - sortedTimes[high] <= sortedTimes[low] - target ? high : low
}
...
function BacktestChart({
  ...
  const chartTimes = useMemo(() => equity.map((p) => p.ts), [equity])

  // Find trade points to mark on chart
  const tradeMarkers = useMemo(() => {
    if (!trades.length || !equity.length) return []

    const relevant: BacktestTradeEvent[] = []
    for (let i = trades.length - 1; i >= 0 && relevant.length < 30; i--) {
      const trade = trades[i]
      if (trade.action.includes('open') || trade.action.includes('close')) {
        relevant.push(trade)
      }
    }
    relevant.reverse()

    return relevant.map((trade) => {
      const closestIdx = findClosestIndexSorted(chartTimes, trade.ts)
      const closest = equity[closestIdx]
      return {
        ts: closest.ts,
  ...
  }, [trades, equity, chartTimes])

  const chartIndexByTs = useMemo(() => {
    const map = new Map<number, number>()
    for (let i = 0; i < chartData.length; i++) {
      map.set(chartData[i].ts, i)
    }
    return map
  }, [chartData])
      ...
      <ResponsiveContainer width="100%" height="100%">
        ...
        >
          ...
          {tradeMarkers.map((marker, idx) => (
            <ReferenceDot
              key={`${marker.ts}-${idx}`}
              x={chartIndexByTs.get(marker.ts) ?? 0}
              y={marker.equity}
              r={4}
...
function CandlestickChartComponent({
  ...
  const [klines, setKlines] = useState<BacktestKlinesResponse['klines']>([])
  const markersApiRef = useRef<ISeriesMarkersPluginApi<Time> | null>(null)

  const latestKlinesRequestIdRef = useRef(0)
  ...
  const klineTimes = useMemo(() => klines.map((k) => k.time), [klines])
  ...
  // Fetch klines with cancellation and "latest wins"
  useEffect(() => {
    if (!selectedSymbol || !runId) return

    const controller = new AbortController()
    const requestId = ++latestKlinesRequestIdRef.current
    ...
    api
      ...
      .then((data: BacktestKlinesResponse) => {
        ...
        if (requestId !== latestKlinesRequestIdRef.current) return

        setKlines(Array.isArray(data.klines) ? data.klines : [])
        setIsLoading(false)
      })
      .catch((err) => {
        if (controller.signal.aborted) return
  ...
  // Update markers when trades or klines change
  useEffect(() => {
    const series = candleSeriesRef.current
    if (!series) return
    if (!klineTimes.length) return
    ...
    const markers: SeriesMarker<UTCTimestamp>[] = symbolTrades
      ...
      .map((trade) => {
        ...
        const closestIdx = findClosestIndexSorted(klineTimes, tradeTime)
        const closest = klines[closestIdx]
        ...
        return {
          time: (closest?.time ?? klineTimes[0]) as UTCTimestamp,
          position: isOpen
            ? isLong
              ? ('belowBar' as const)
              : ('aboveBar' as const)
  ...
  }, [symbolTrades, klines, klineTimes])
...
export function BacktestPage() {
  ...
  const strategyHasDynamicCoins = useMemo(() => {
    ...
    if (
      ...
      (coinSource.use_coin_pool || coinSource.use_oi_top)
    ) {
      return true
    }
                            ...
                            style={{
                              background:
                                formState.prompt === p
                                  ...
                                  : '#1E2329',
                              border: `1px solid ${
                                formState.prompt === p ? '#F0B90B' : '#2B3139'
                              }`,
                              color:
                    ...
                    style={{
                      background:
                        run.run_id === selectedRunId
                          ...
                          : '#1E2329',
                      border: `1px solid ${
                        run.run_id === selectedRunId ? '#F0B90B' : '#2B3139'
                      }`,
                    }}
                  >
